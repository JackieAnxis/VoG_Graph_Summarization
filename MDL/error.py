...
class Error:
    ...
    def __init__(self, graph, err=None):
        ...
        if err is None:
            ...
            # ! "unmodelled(Errors)" means the edge exists, but our model doest not model it.
            # a list with $numNodes items, each item is an empty set
            # it means all edges are regarded as unmodelled errors in the beginning
            self.unmodelled = [set(x) for x in graph.edges]
            self.numUnmodelledErrors = graph.numEdges

            # ! "modelled(Errors)" means the edge does not exist, but our model models it.
            # a list with $numNodes items, each item is an empty set
            # it means no edge is modelled as error in the beginning
    ...
    def cover(self, i, j):
        # cover means this edges has been modeled yet.
    ...
    def delUnmodelledError(self, i, j):
        self.unmodelled[min(i, j) - 1].remove(max(i, j) - 1)  # if not in set, KeyError will be raised
        self.numUnmodelledErrors -= 1
    ...
    def listError(self):
        for idx in range(len(self.unmodelled)):
            if len(self.unmodelled[idx]) > 0:
                print(idx+1, "+: "+str([x+1 for x in self.unmodelled[idx]]),
                      "-: "+str([x+1 for x in self.modelled[idx]]), end=' ')
