...
def composition(n, k):
    # C_{n-1}^{k-1}, choose k-1 from total n-1 items
...
def weakcomposition(n, k):
    # C_{n+k-1}^{k-1}, choose n+k-1 from total n-1 items
...
def L(G, M, errorEnc):
    ...
    # encode number of structures we're encoding with
    model_cost = LN(M.numStructs + 1)
    # encode the number per structure
    # number of structures & number of structure types
    # week composition
    model_cost += LwC(M.numStructs, M.numStrucTypes)
...
def LfullClique(c, M, G, E):
    """length of encoding the full clique structure
    Args:
        c: the Clique structure
        M: the Model
        G: the entire Graph
        E: the Error

    Returns:
        cost: the encoding cost
...
def coverFullClique(G, E, c):
    """ update $E(Error) of encoding the structure $c as full clique
    Edges formed by all combinations of nodes will be covered.
    If one edge exists, it is modelled correctly, it should not be an unmodelled error (remove from unmodelledError).
    If one edge does not exist, it is modelled mistakenly, it should be a modelling error (add into modellingError).
    """
    # c.nodes is ordered
    # we model it as a full clique, so all node combinations should be covered
            ...
            if not E.isExcluded(i, j):
                ...
                if not E.isCovered(i, j):
                    ...
                    if G.hasEdge(i, j):
                        ...
                        # unmodelledError set is initialized by existing edges
                    ...
                    # edge is already modelled, update modelling error
                    if G.hasEdge(i, j) and E.isModellingError(i, j):
                        ...
                        E.addModellingError(i, j)
                    # dont need to test unModelledError, because all combinations of nodes are covered in full clique model
    return
...
def LnearClique(c, M, G, E):
    ...
    cost += LU(G.numNodes, c.numNodes)  # encode node ids
    if cnt0 + cnt1 > 0:
        # encode probability of a 1 (cnt0+cnt1 is number of cells we describe, upperbounded by numnodes 2)
...
def coverNearClique(G, E, c):
    """update $E(Error) of encoding the structure $c as near clique
    If one edge exists, it is modelled correctly, it should not be an unmodelled error (remove from unmodelledError).
    No edge will be regarded as a modelling error (because it is a *near* clique).
    ...
    """
    # c.nodes is ordered
    cnt0 = 0  # count exclude edges
    cnt1 = 0  # count include edges
    for i_idx in range(c.numNodes):
        i = c.nodes[i_idx]
        for j_idx in range(i_idx+1, c.numNodes):
            j = c.nodes[j_idx]
        ...
        if not E.isExcluded(i, j):
            ...
            if not E.isCovered(i, j):
                ...
                if G.hasEdge(i, j):
                    ...
                    E.addModellingError(i, j)
                E.cover(i, j)  # cover means this edges has been modeled yet.
...
def LjellyFish(jf, M, G, E):
    ...
    # number of spokes per core node
    cost += LN(jf.numSpokeSum) + LC(jf.numSpokeSum, jf.numCores)
    # spoke ids (-no- overlap between sets!)
    cost += LU(G.numNodes - jf.numCores, jf.numSpokeSum)
    return cost
...
def LcorePeriphery(cp, M, G, E):
    ...
    # identify spoke-nodes
    cost += cp.numSpokes * log(G.numNodes - cp.numCores, 2)
    return cost
...
def LcorePeripheryA(cp, M, G, E):
    ...
    cost += LU(G.numNodes - cp.numCoreNodes,
               cp.numSpokes)  # identify spoke-nodes
    return cost
