%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Encode given graph as chain: Start from a node with deg 1 (p_rand) and find its furthest node using BFS. Then, starting from the found node (p_init), redo BFS and find its furthest node (p_fin). Report the shortest path between p_init and p_fin. If there are extra nodes in the shortest path, report them after the path (separating with comma from the path nodes).
% DESCRIPTION OF SOME VARS:
%   E = M xor Asmall, error matrix (xor between true model and adjacency mat)
%
% @param{Asmall}: the adjacent matrix of the subgraph
% @param{N_tot}: # of whole graph nodes (total)
%
% @return{MDLcost}: the cost of encoding Asmall as a chain
%? @return{chainExt}:
